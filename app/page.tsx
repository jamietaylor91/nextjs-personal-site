"use client";
import { motion } from "motion/react";
import { Spotlight } from "@/components/ui/spotlight";
import { Magnetic } from "@/components/ui/magnetic";

import { PROJECTS, WORK_EXPERIENCE, EMAIL, SOCIAL_LINKS } from "./data";
import DownloadResumeButton from "@/components/ui/download-resume-button";

const VARIANTS_CONTAINER = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.15,
    },
  },
};

const VARIANTS_SECTION = {
  hidden: { opacity: 0, y: 20, filter: "blur(8px)" },
  visible: { opacity: 1, y: 0, filter: "blur(0px)" },
};

const TRANSITION_SECTION = {
  duration: 0.3,
};

function MagneticSocialLink({
  children,
  link,
}: {
  children: React.ReactNode;
  link: string;
}) {
  return (
    <Magnetic springOptions={{ bounce: 0 }} intensity={0.3}>
      <a
        href={link}
        className="group relative inline-flex shrink-0 items-center gap-[1px] rounded-full bg-zinc-800 px-2.5 py-1 text-sm text-zinc-100 transition-colors duration-200 hover:bg-zinc-700"
      >
        {children}
        <svg
          width="15"
          height="15"
          viewBox="0 0 15 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="h-3 w-3"
        >
          <path
            d="M3.64645 11.3536C3.45118 11.1583 3.45118 10.8417 3.64645 10.6465L10.2929 4L6 4C5.72386 4 5.5 3.77614 5.5 3.5C5.5 3.22386 5.72386 3 6 3L11.5 3C11.6326 3 11.7598 3.05268 11.8536 3.14645C11.9473 3.24022 12 3.36739 12 3.5L12 9.00001C12 9.27615 11.7761 9.50001 11.5 9.50001C11.2239 9.50001 11 9.27615 11 9.00001V4.70711L4.35355 11.3536C4.15829 11.5488 3.84171 11.5488 3.64645 11.3536Z"
            fill="currentColor"
            fillRule="evenodd"
            clipRule="evenodd"
          ></path>
        </svg>
      </a>
    </Magnetic>
  );
}

export default function Personal() {
  return (
    <motion.main
      className="space-y-24"
      variants={VARIANTS_CONTAINER}
      initial="hidden"
      animate="visible"
    >
      <motion.section
        variants={VARIANTS_SECTION}
        transition={TRANSITION_SECTION}
      >
        <div className="flex-1">
          <p className="text-zinc-400">
            I build software products from early idea to launch, combining
            product judgment, full-stack engineering, and AI integration. My
            work sits at the intersection of strategy, execution, and customer
            feedback, with a focus on turning ambiguous problems into useful,
            measurable products.
          </p>
        </div>
      </motion.section>
      <motion.section>
        <div className="flex-1">
          <DownloadResumeButton />
        </div>
      </motion.section>
      <motion.section
        variants={VARIANTS_SECTION}
        transition={TRANSITION_SECTION}
      >
        <h3 className="mb-5 text-lg font-medium">Portfolio</h3>
        <div className="flex flex-col space-y-2">
          {PROJECTS.map((project) => (
            <a
              className="relative overflow-hidden rounded-2xl bg-zinc-600/30 p-[1px]"
              href={project.link}
              target="_blank"
              rel="noopener noreferrer"
              key={project.id}
            >
              <Spotlight
                className="from-zinc-100 via-zinc-200 to-zinc-50 blur-2xl"
                size={64}
              />
              <div className="relative h-full w-full rounded-[15px] bg-zinc-950 p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-2">
                    <h4 className="font-normal text-zinc-100">
                      {project.name}
                    </h4>
                    <p className="text-sm leading-6 text-zinc-400">
                      {project.description}
                    </p>
                  </div>
                  <svg
                    width="15"
                    height="15"
                    viewBox="0 0 15 15"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="mt-1 h-3.5 w-3.5 shrink-0 text-zinc-400"
                  >
                    <path
                      d="M3.64645 11.3536C3.45118 11.1583 3.45118 10.8417 3.64645 10.6465L10.2929 4L6 4C5.72386 4 5.5 3.77614 5.5 3.5C5.5 3.22386 5.72386 3 6 3L11.5 3C11.6326 3 11.7598 3.05268 11.8536 3.14645C11.9473 3.24022 12 3.36739 12 3.5L12 9.00001C12 9.27615 11.7761 9.50001 11.5 9.50001C11.2239 9.50001 11 9.27615 11 9.00001V4.70711L4.35355 11.3536C4.15829 11.5488 3.84171 11.5488 3.64645 11.3536Z"
                      fill="currentColor"
                      fillRule="evenodd"
                      clipRule="evenodd"
                    ></path>
                  </svg>
                </div>
              </div>
            </a>
          ))}
        </div>
      </motion.section>

      <motion.section
        variants={VARIANTS_SECTION}
        transition={TRANSITION_SECTION}
      >
        <h3 className="mb-5 text-lg font-medium">Work Experience</h3>
        <div className="flex flex-col space-y-2">
          {WORK_EXPERIENCE.map((job) => {
            const Card = job.link ? "a" : "div";

            return (
              <Card
                className="relative overflow-hidden rounded-2xl bg-zinc-600/30 p-[1px]"
                href={job.link}
                target={job.link ? "_blank" : undefined}
                rel={job.link ? "noopener noreferrer" : undefined}
                key={job.id}
              >
              <Spotlight
                className="from-zinc-100 via-zinc-200 to-zinc-50 blur-2xl"
                size={64}
              />
              <div className="relative h-full w-full rounded-[15px] bg-zinc-950 p-4">
                <div className="relative flex w-full flex-col gap-3 sm:flex-row sm:justify-between">
                  <div className="space-y-1">
                    <h4 className="font-normal text-zinc-100">
                      {job.title}
                    </h4>
                    <p className="text-zinc-400">
                      {job.company}
                    </p>
                  </div>
                  <p className="shrink-0 text-sm text-zinc-400">
                    {job.start} - {job.end}
                  </p>
                </div>
                {job.description ? (
                  <p className="mt-4 text-sm leading-6 text-zinc-400">
                    {job.description}
                  </p>
                ) : null}
                {job.highlights ? (
                  <ul className="mt-4 list-disc space-y-2 pl-5 text-sm leading-6 text-zinc-400">
                    {job.highlights.map((highlight) => (
                      <li key={highlight}>{highlight}</li>
                    ))}
                  </ul>
                ) : null}
              </div>
              </Card>
            );
          })}
        </div>
      </motion.section>

      <motion.section
        variants={VARIANTS_SECTION}
        transition={TRANSITION_SECTION}
      >
        <h3 className="mb-5 text-lg font-medium">Connect</h3>
        <p className="mb-5 text-zinc-400">
          Feel free to contact me at{" "}
          <a className="text-zinc-300 underline" href={`mailto:${EMAIL}`}>
            {EMAIL}
          </a>
        </p>
        <div className="flex items-center justify-start space-x-3">
          {SOCIAL_LINKS.map((link) => (
            <MagneticSocialLink key={link.label} link={link.link}>
              {link.label}
            </MagneticSocialLink>
          ))}
        </div>
      </motion.section>
    </motion.main>
  );
}
